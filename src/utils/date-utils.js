 ...
 * @param y Year
 * @return Number
 * @eg:var count = lYearDays(1987) ; // count=384
 */
 ...
 * @eg:var solarMonthDay = solarDays(1987) // solarMonthDay=30
 */
 ...
 */
export function toChinaMonth(m) {
  if (m > 12 || m < 1) return -1 // 若参数错误 返回-1
  let s = nStr3[m - 1]
  s += '\u6708' // 加上月字
  return s
 ...
 * @param m  lunar month
 * @param d  lunar day
 * @param isLeapMonth [如果是农历闰月第四个参数赋值true即可]
 * @return JSON object
...
export function lunar2solar(y, m, d, isLeapMonth) {
  ...
  let calObj = new Date((offset + d - 31) * 86400000 + stamp)
